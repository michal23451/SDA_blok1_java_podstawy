...
public class Storage {
    ...
    public void editProduct(){
        ...
        if (counter == productArray.length) {
            System.out.println("Tablica nie ma danych. Najpierw ją uzupełnj!");
            return;
        }


        System.out.print("Podaj indeks produktu do edycji: ");
        Scanner myScanner = (new Scanner(System.in));
        int index = myScanner.nextInt();

        while (index >= productArray.length || productArray[index] == null) {
            if (index >= productArray.length) {
                System.out.print("Za duża wartość - indeks nie może być większy niż " + (productArray.length - 1) + ". Podaj poprawną wartość: ");
            }
            else if (productArray[index] == null){
                System.out.print("Na pozycji o indeksie " + index + " nic nie ma. Podaj inną wartość: ");
            }
            index = myScanner.nextInt();
        }
        myScanner.nextLine();

        System.out.println("Chcesz edytować produkt w tablicy na indeksie: " + index);
        Product p = new Product();
        System.out.print("Podaj nazwę produktu: ");
        p.setName(myScanner.nextLine());
        System.out.print("Podaj ilość produktu: ");
